...
- 对持仓股和推荐股输出具体建仓/加仓区间、减仓价、止损价和目标价
- 内置模拟交易账户，默认 100 万起始资金；软件会按推荐策略自动纸面交易，记录交易后复盘、盈亏和策略参数调优
- 生成每日复盘与持仓简报，汇总指数、自选股/持仓股新闻、行业热点、持仓/建仓策略
- 按 9:15、10:30、11:30、14:30、15:00 固定节奏推送开盘早知道、盘中快讯和今日总结
...
右侧“标的查询”支持按代码或名称查询真实 A 股、港股和常用指数。查询结果可以直接加入自选股或指数池，并写回 `config/local.json`；自选股也可以在页面里直接删除。

模拟交易的候选池不只来自自选股：系统会合并自选股、持仓股、`etf_pools` 里的 ETF/相关股票、新闻热点人气候选，以及近期报告里的推荐观察池。指数只用于市场判断，不会被模拟账户直接买入。

“模拟交易”页面用于查看软件自动纸面交易的状态，不需要手动录入交易。后台调度器会在固定报告节奏后自动评估并交易；页面支持暂停/恢复自动运行、重置起始资金、按时间筛选交易与复盘记录。
